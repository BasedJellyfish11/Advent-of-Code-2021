...
namespace AdventOfCode05
{
    ...
    internal static class AdventOfCode05
    {
        ...
        private static List<(int, int)>? findIntegerPointsOfSegment(Segment segment)
        {
            ...
            {
                result.AddRange(Enumerable.Range(Math.Min(segment.startY, segment.endY), Math.Abs(segment.startY - segment.endY) + 1).Select(y => (segment.startX, y)));
                return result;
            }
        ...
        private static async Task Main()
        {
            ...
            HashSet<Segment> segments = new();
            
            foreach (string line in input)
            {
                Match match = Regex.Match(line ,@"^(\d+),(\d+) -> (\d+),(\d+)$", RegexOptions.Compiled);
                int[] numbers = match.Groups.Values.Skip(1).Select(x => int.Parse(x.Value)).ToArray();
                segments.Add(new Segment() { startX = numbers[0], startY = numbers[1], endX = numbers[2], endY = numbers[3] });
            }
